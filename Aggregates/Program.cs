    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            using (var customerRepository = new CustomerRepository())
            {
                ...
                var paypalCustomer = customerFactory.Create("John Doe", new PaypalPaymentMethod("john.doe@gmail.com"));
                customerRepository.Add(paypalCustomer);

                var stripeCustomer = customerFactory.Create("Jane Smith", new StripePaymentMethod("1234567890"));
                customerRepository.Add(stripeCustomer);

                var cardCustomer = customerFactory.Create("Vasiliy Petrov",
                    new CardPaymentMethod("1234567890", "Vasiliy Petrov", "01/2022", CardType.Mir));
